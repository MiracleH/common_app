...
import 'package:common_app/constant/cache_key.dart';
import 'package:common_app/model/user/user.dart';
...
class AccountService {
  ...
  Future _initAsync() async {
    ...
    _user = SpUtil.getObj(CacheKey.userDataKey, (m) => User.fromJson(m));
  }
  ...
  User get currentUser {
    if (_user == null) {
      _user =
          SpUtil.getObj(CacheKey.userDataKey, (m) => User.fromJson(m));
    }
    return _user;
  }
  ...
  void loginUser(User user, {String rawLogin = ''}) {
    // 存储登陆账号
    _rawLogin = rawLogin ?? '';
    SpUtil.putString(CacheKey.loginKey, rawLogin);

    // 存储用户数据
    _user = user;
    SpUtil.putObject(CacheKey.userDataKey, user);
  }
  ...
  void logoutUser() {
    // 清除账号信息
    _rawLogin = '';
    SpUtil.remove(CacheKey.loginKey);
  ...
  String get rawLogin {
    if (_rawLogin == null) {
      _rawLogin = SpUtil.getString(CacheKey.loginKey);
    }
    return _rawLogin;
  }
}
